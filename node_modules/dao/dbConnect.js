...
function insertCarInfo(client,vin,engineNo,model,color,address,carCard,cityName,modelYear,purchaseDate,carType,memo,status,createUserId,createTime,callback){
	var promise = new Promise(function(resolve,reject){
		var result = null;
		client.connect();
		client.query('INSERT INTO car_info(vin,engine_no,model,color,address,car_card,city_name,model_year,purchase_date,car_type,memo,status,create_user_id,create_time) value(?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[vin,engineNo,model,color,address,carCard,cityName,modelYear,purchaseDate,carType,memo,status,createUserId,createTime],function(err,result){
			if(err){ 
                console.log("err");
                reject(err); 
            }
